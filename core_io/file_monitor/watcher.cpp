...
void
DirectoryMonitor::on(qb::io::async::event::file const &event) {
    ...
    // Check attributes
    if (event.attr.st_mode != event.prev.st_mode || event.attr.st_uid != event.prev.st_uid || event.attr.st_gid != event.prev.st_gid) {
    ...
    else if (_last_mtimes.find(file_path) == _last_mtimes.end() || event.attr.st_nlink > event.prev.st_nlink) {
...
void
DirectoryMonitor::startWatching(const std::filesystem::path &path, qb::duration interval) {
    start(path, interval);
}

void
DirectoryMonitor::stopWatching() {
    disconnect();
}
...
DirectoryWatcher::on(WatchDirectoryRequest &request) {
    ...
    if (!fs::exists(normalized_path) || !fs::is_directory(normalized_path)) {
        ...
        push<WatchDirectoryResponse>(request.requestor, normalized_path, false, "Path does not exist or is not a directory");
        return;
    }
    ...
    if (std::find(watch->subscribers.begin(), watch->subscribers.end(), request.requestor) == watch->subscribers.end()) {
    ...
    qb::io::async::callback([this, normalized_path, watch, request]() {
        ...
        push<WatchDirectoryResponse>(request.requestor, normalized_path, success, success ? "" : "Failed to set up directory watch");
...
DirectoryWatcher::on(UnwatchDirectoryRequest &request) {
    ...
    if (it != _watched_directories.end()) {
        ...
        subscribers.erase(std::remove(subscribers.begin(), subscribers.end(), request.requestor), subscribers.end());
...
void
DirectoryWatcher::on(qb::KillEvent &) {
...
bool
DirectoryWatcher::setupDirectoryWatch(const std::filesystem::path &path, std::shared_ptr<WatchInfo> watch, bool recursive) {
    try {
        // Create the DirectoryMonitor with a callback
        watch->watcher = std::make_unique<DirectoryMonitor>([this, path](const std::string &file_path, FileEventType event_type) {
...
void
DirectoryWatcher::publishFileEvent(const std::filesystem::path &file_path, FileEventType event_type) {
    ...
    if (watch) {
        // Log the event
        qb::io::cout() << "File event: " << eventTypeToString(event_type) << " - " << file_path_str << std::endl;
...
int
DirectoryWatcher::countWatchedFiles(std::shared_ptr<WatchInfo> watch) {
    int count = 0;
