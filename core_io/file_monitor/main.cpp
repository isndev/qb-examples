 ...
 *         `DirectoryWatcher` directly sends events to `FileProcessor` based on its `onInit` setup in some designs, or `FileProcessor` might
 * subscribe). *Correction based on example code: `DirectoryWatcher` sends `FileEvent` to all its registered subscribers for that path.
 * `FileProcessor` is not explicitly shown subscribing in `main.cpp`, but `ClientActor` requests a watch and receives `FileEvent`s. The
 * `FileProcessor` also receives `FileEvent`s - likely the example intends for the `DirectoryWatcher` to broadcast or for the processor to
 * subscribe, but the most direct path shown is `ClientActor` sending `WatchDirectoryRequest` to `DirectoryWatcher` which then sends
 * `FileEvent`s back to its subscribers. In this specific `main.cpp`, the `FileProcessor` is created but not directly linked for `FileEvent`s
 * from `DirectoryWatcher` via a subscription setup in `main`. It seems the `FileProcessor` is expected to be a general recipient of
 * `FileEvent`s if they are broadcast or if it registers itself as a subscriber, which is not explicitly shown here. The current header in
 * `processor.cpp` implies it handles `FileEvent`. This `main.cpp` doesn't directly wire them up beyond creating instances.* Let's assume
 * `DirectoryWatcher` will broadcast or `ClientActor` might forward. Re-evaluating: The `DirectoryMonitor` (helper for `DirectoryWatcher`) takes
 * a callback which `DirectoryWatcher::publishFileEvent` uses to `push<FileEvent>` to all its subscribers for a given path. The `ClientActor`
 * subscribes by sending `WatchDirectoryRequest`. The `FileProcessor` would also need to send such a request or be added as a default subscriber
 * if it were to receive these events directly. Let's assume the `DirectoryWatcher` is designed to send `FileEvent`s to interested parties like
 * `ClientActor` and `FileProcessor` if they were to register for specific paths.
 *
...
public:
    ...
    ClientActor(qb::ActorId watcher_id, std::filesystem::path test_dir, int duration = 30)
        : _watcher_id(watcher_id)
        , _test_directory(std::move(test_dir))
        , _test_duration_seconds(duration) {
