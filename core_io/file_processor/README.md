...
* **The manager's response-forwarding handlers are dead as wired.** `FileManager` registers and implements
  ...
  the worker pushes the response directly to `request.requestor`, which is the `ClientActor` (`file_worker.h:140`,
  `:189`), because `ClientActor` already fills that field when it creates the request (`main.cpp:178`, `:189`). The
...
* **"Offload blocking I/O" is not what happens.** Both workers wrap their file work in
  `qb::io::async::callback(lambda)` with **one** argument (`file_worker.h:102`, `:158`). That overload schedules
...
> **The delayed callbacks in `ClientActor` capture `this` — do not copy that shape.**
> `main.cpp:92` (`startTests` after 500 ms) and `main.cpp:201-206` (broadcast the kill after 1 s) both use
> `qb::io::async::callback(f, delay)`, which heap-allocates a `Timeout` owned by the **event loop**, not by the actor
> (`qb/src/qb/io/async/io.h:389`). Nothing cancels it if the actor dies first, and the lambda would then dereference
> freed memory. The lifetime-bound form is:
>
> ```cpp
> spawn([copies_by_value](qb::ScopedCoroContext ctx) -> qb::io::async::task<void> {
>     co_await ctx.sleep(std::chrono::milliseconds(500));
>     ctx.template push<StartTestsEvent>();
> });
> ```
>
> `spawn` is `qb/src/qb/core/Actor.h:1238-1239`; `Actor::kill()` cancels its scope at
> `qb/src/qb/core/Actor.cpp:283-289`. Never capture `this`.

## QB Features Demonstrated

* **Core (`qb-core`)**:
    * `qb::Actor` for all three roles; `qb::Main` for engine setup and lifecycle.
    * Multi-core deployment with `engine.addActor<T>(core_id, ...)` (`main.cpp:223-240`).
    * Coroutine initialisation: `qb::io::async::task<bool> onInit()` with `co_return true`
      (`file_manager.h:83-87`, `file_worker.h:80-88`, `main.cpp:82-95`).
