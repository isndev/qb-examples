...
This directory holds static assets and related frontend code for the `08_static_files.cpp` example, demonstrating file
...
* **Frontend for API Interaction**: `upload.html` and `upload.js` provide a user interface to test the file upload API (
  `/api/upload`) and file management APIs (`/api/files`) implemented in `08_static_files.cpp`.
...
* **`upload.js`**: Contains the JavaScript logic for handling file uploads, listing files, and deleting files via the
  API endpoints provided by `08_static_files.cpp`.
...
demonstrate HTTP/2 features in the QB framework `12_http2_server.cpp` example.
...
These files create an interactive web application served by `12_http2_server.cpp` to showcase and allow testing of
...
2. Use `qb::http::StaticFilesMiddleware` to serve files from the `examples/qbm/http/resources/http2/` directory as its
...
- The example servers (`08_static_files.cpp`, `12_http2_server.cpp`) serve these files directly from the filesystem
