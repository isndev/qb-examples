...
public:
    ...
    qb::io::async::task<void> setup_inventory() {
        auto cout = qb::io::cout();
        ...
        qb::io::async::callback([this]() {
            spawn([this](qb::ScopedCoroContext) -> qb::io::async::task<void> {
                co_await demonstrate_redis_operations();
            });
        }, std::chrono::seconds(1));
    }

    // Display current inventory
    qb::io::async::task<void> display_inventory() {
        auto cout = qb::io::cout();
    ...
    void on(const OrderRequestEvent& event) {
        ...
        spawn([this, product_id, order_qty, sender_id](qb::ScopedCoroContext)
            ...
            if (!qty_r.ok() || !qty_r.result().has_value()) {
                push<OrderResultEvent>(sender_id, product_id, order_qty,
            ...
            if (current_qty < order_qty) {
                cout << "Not enough stock for " << product_id
                     ...
                     << ", Requested: " << order_qty << std::endl;

                push<OrderResultEvent>(sender_id, product_id, order_qty, false,
            ...
            // Record the order
            std::string order_id = "order:" + product_id + ":" +
                                   std::to_string(std::time(nullptr));
            [[maybe_unused]] auto oh1 = co_await _redis.hset(order_id, "product_id", product_id);
            [[maybe_unused]] auto oh2 = co_await _redis.hset(order_id, "quantity",
                                 std::to_string(order_qty));
            [[maybe_unused]] auto oh3 = co_await _redis.hset(order_id, "timestamp",
                                 std::to_string(std::time(nullptr)));

            cout << "Order processed successfully! "
                 << order_qty << " units of " << product_id
                 << " ordered." << std::endl;

            push<OrderResultEvent>(sender_id, product_id, order_qty, true,
                                   "Order processed successfully");
        });
    }

    // Demonstrate simple Redis key-value operations
    qb::io::async::task<void> demonstrate_redis_operations() {
        auto cout = qb::io::cout();
        ...
        // Set a timestamp
        [[maybe_unused]] auto ts = co_await _redis.set("transaction:last_access",
                            std::to_string(std::time(nullptr)));

        // Add to a list
        [[maybe_unused]] auto lp = co_await _redis.lpush("transaction:logs",
                              "Operation executed at " +
                              std::to_string(std::time(nullptr)));
...
public:
    ...
    qb::io::async::task<bool> onInit() override {
        auto cout = qb::io::cout();
        cout << "OrderClientActor [" << _client_id << "] initialized"
             << std::endl;
    ...
    void on(const ShutdownEvent&) {
        auto cout = qb::io::cout();
        cout << "OrderClientActor [" << _client_id << "] shutting down"
             << std::endl;
        kill();
    }
};
...
public:
    ...
    void create_clients() {
        ...
        for (int i = 1; i <= _clients_to_create; ++i) {
            ...
            auto h = addRefActor<OrderClientActor>(
