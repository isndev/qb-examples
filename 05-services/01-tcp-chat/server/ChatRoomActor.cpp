/**
 * @file examples/05-services/01-tcp-chat/server/ChatRoomActor.cpp
 ...
 */
void
ChatRoomActor::on(AuthEvent &evt) {
 ...
 */
void
ChatRoomActor::on(ChatEvent &evt) {
    auto session_id = evt.session_id;
 ...
 */
void
ChatRoomActor::on(DisconnectEvent &evt) {
    auto session_id = evt.session_id;
    auto it         = _sessions.find(session_id);
    if (it == _sessions.end())
        return;
