# Example: Distributed File Processor (`qb-example-services-file-pipeline`)
...
> ```
>
> `spawn` is `qb/src/qb/core/Actor.h:1287-1288`; `Actor::kill()` cancels its scope at
> `qb/src/qb/core/Actor.cpp:439-445`. Never capture `this` in the coroutine — the work moves into
...
1. **Build** — from the workspace root (the superproject force-enables `QB_BUILD_EXAMPLES`):
   ```bash
   cmake --preset dev
   cmake --build build/presets/dev --target qb-example-services-file-pipeline
   ```
   The executable lands in `build/presets/dev/examples/05-services/03-file-pipeline/`.
