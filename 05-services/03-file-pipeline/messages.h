/**
 * @file examples/05-services/03-file-pipeline/messages.h
...
struct ReadFileResponse : public qb::Event {
    ...
    ReadFileResponse(const char *path, std::shared_ptr<std::vector<char>> content, bool ok, const char *error, uint32_t id)
        : filepath(path)
        , data(content)
        , success(ok)
        , error_msg(error)
...
struct WriteFileRequest : public qb::Event {
    ...
    WriteFileRequest(const char *path, std::shared_ptr<std::vector<char>> content, qb::ActorId req_id, uint32_t id)
        : filepath(path)
        , data(content)
        , requestor(req_id)
        , request_id(id) {}
...
struct WorkerAvailable : public qb::Event {
    ...
    explicit WorkerAvailable(qb::ActorId id)
        : worker_id(id) {}
};
