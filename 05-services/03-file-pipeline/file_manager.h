/**
 * @file examples/05-services/03-file-pipeline/file_manager.h
...
public:
    ...
    on(ReadFileRequest &request) {
        ...
        qb::io::cout() << "FileManager received a read request for " << request.filepath.c_str() << " (ID: " << request.request_id << ")"
                       << std::endl;
    ...
    on(WriteFileRequest &request) {
        ...
        qb::io::cout() << "FileManager received a write request for " << request.filepath.c_str() << " (ID: " << request.request_id << ")"
                       << std::endl;
        ...
        if (!_available_workers.empty()) {
            ...
            push<WriteFileRequest>(worker_id, request.filepath.c_str(), request.data, request.requestor, request.request_id);
        } else {
            // Otherwise, queue the request
     ...
     */
    void
    on(WorkerAvailable &msg) {
        qb::io::cout() << "FileManager received an availability notification from worker " << msg.worker_id << std::endl;
        ...
        if (!_read_requests.empty()) {
            ...
            qb::io::cout() << "FileManager assigns a queued read task to worker " << msg.worker_id << std::endl;

            push<ReadFileRequest>(msg.worker_id, request.filepath.c_str(), request.requestor, request.request_id);
        } else if (!_write_requests.empty()) {
            // Then write operations
            WriteFileRequest request = _write_requests.front();
            _write_requests.pop();

            qb::io::cout() << "FileManager assigns a queued write task to worker " << msg.worker_id << std::endl;

            push<WriteFileRequest>(msg.worker_id, request.filepath.c_str(), request.data, request.requestor, request.request_id);
        } else {
    ...
    on(ReadFileResponse &response) {
        qb::io::cout() << "FileManager received a read response for " << response.filepath.c_str() << " (ID: " << response.request_id << ")"
                       << std::endl;

        // Forward the response to the requesting actor
        push<ReadFileResponse>(response.getSource(), response.filepath.c_str(), response.data, response.success, response.error_msg.c_str(),
                               response.request_id);
    }
    ...
    on(WriteFileResponse &response) {
        qb::io::cout() << "FileManager received a write response for " << response.filepath.c_str() << " (ID: " << response.request_id << ")"
                       << std::endl;

        // Forward the response to the requesting actor
        push<WriteFileResponse>(response.getSource(), response.filepath.c_str(), response.bytes_written, response.success,
                                response.error_msg.c_str(), response.request_id);
    }
