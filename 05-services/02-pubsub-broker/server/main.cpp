/**
 * @file examples/05-services/02-pubsub-broker/server/main.cpp
 * @tier 05-services
 * @teaches Topic fan-out without copying the payload: one message body behind a shared_ptr and
...
main(int argc, char *argv[]) {
    ...
    try {
        ...
        auto server_ids = engine.core(1).builder().addActor<ServerActor>(topic_manager_id).addActor<ServerActor>(topic_manager_id).idList();
        ...
        engine.core(0).builder().addActor<AcceptActor>(qb::io::uri{"tcp://0.0.0.0:12345"}, server_ids);
