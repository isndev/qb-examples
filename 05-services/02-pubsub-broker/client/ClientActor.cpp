/**
 * @file examples/05-services/02-pubsub-broker/client/ClientActor.cpp
 ...
 */
void
ClientActor::on(const broker::Message &msg) {
    switch (msg.type) {
...
ClientActor::connect() {
    qb::io::async::tcp::connect<qb::io::tcp::socket>(
        ...
        std::chrono::duration_cast<qb::duration>(std::chrono::duration<double>(CONNECT_TIMEOUT)));
}
 ...
 */
void
ClientActor::sendSubscribe(const std::string &topic) {
    if (!_connected) {
 ...
 */
void
ClientActor::sendUnsubscribe(const std::string &topic) {
    if (!_connected) {
        qb::io::cout() << "Not connected to server. Command discarded." << std::endl;
        return;
    }
 ...
 */
void
ClientActor::sendPublish(const std::string &topic, const std::string &message) {
    if (!_connected) {
        qb::io::cout() << "Not connected to server. Command discarded." << std::endl;
        return;
    }
 ...
 */
void
ClientActor::on(const BrokerInputEvent &evt) {
 ...
 */
void
ClientActor::processCommand(const std::string &command) {
