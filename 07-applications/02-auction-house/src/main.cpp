/**
 * @file examples/07-applications/02-auction-house/src/main.cpp
 * @tier 07-applications
 * @teaches Work that has to happen BEFORE the engine exists: the schema is created by a
 *          coroutine driven with run_sync on the main thread, and only then do the actors
 *          start — because an actor that finds no table has nowhere to put the failure.
 * @demonstrates qb::io::async::init, qb::io::async::run_sync, qb::io::async::task<void>,
 *               qb::pg::tcp::database, qb::Main, qb::Main::registerSignal, engine.core,
 *               setLatency, qb::duration::zero, addActor<T>, qb::ActorId, engine.start,
