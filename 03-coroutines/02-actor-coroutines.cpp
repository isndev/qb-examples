/**
 * @file examples/03-coroutines/02-actor-coroutines.cpp
 * @tier 03-coroutines
 * @teaches A coroutine spawned from inside an actor: capture by value before the first suspend,
 *          talk back only through the context, and let the framework cancel the frame when the
 *          actor dies — the actor may be destroyed while the coroutine is parked.
 * @demonstrates spawn, qb::ScopedCoroContext, qb::io::async::task<void>,
 *               qb::io::async::task<bool>, qb::io::async::sleep, ctx.time, registerEvent<E>,
 *               qb::KillEvent, broadcast<qb::KillEvent>, kill(), qb::Main, addActor<T>
 * @prerequisites 03-coroutines/01-first-coroutine
 * @expect "=== QB Actor + Coroutine Example ==="
 * @expect "requests complete -- initiating shutdown..."
 ...
 *
 * Neither the target nor the binary name is written down anywhere: both are DERIVED from this
 * file's path by `qb_example` in examples/03-coroutines/CMakeLists.txt, which is why the note
 * that used to sit here -- naming a target `actor_coroutine_example` that has never existed in
 * this tree -- has no way to come back.
 *
 * Build:
 *   cmake --preset dev && cmake --build --preset dev --target qb-example-coroutines-actor-coroutines
 *
 * Run:
 *   ./build/presets/dev/examples/03-coroutines/qb-example-coroutines-actor-coroutines
 */
