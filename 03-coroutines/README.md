...
rendezvous), 10 counts source pulls (3 for a synchronous `take(gen, 3)`, 3 for `take`), 11
...
* One measurement worth keeping: `take(gen, 3)` and `take(gen, 3)` each pull exactly **3**, and
