 ...
 * @brief TcpListener – TCP acceptor with round-robin dispatch to AuctionManagers.
 *
 * ## Design
 ...
 * AuctionManager in a round-robin sequence. The AuctionManager then owns the fd
 * and wraps it in an HTTP session.
 *
 * This separation means the hot accept loop is never blocked by HTTP
 * processing, and load is spread evenly across the AuctionManager pool.
 *
 * ## QB patterns used
...
public:
     ...
     * @param targets       Ordered list of AuctionManager actor IDs for round-robin.
     */
    TcpListener(qb::io::uri uri, std::vector<qb::ActorId> targets)
        : _uri(std::move(uri))
        , _targets(std::move(targets)) {}

    bool onInit() final {
        if (_targets.empty()) {
            qb::io::cerr() << "[TcpListener] no AuctionManagers configured\n";
            return false;
        }
        if (!listen(_uri)) {
    ...
    void on(accepted_socket_type &&sock) {
        auto &evt  = push<events::NewConnectionEvent>(_targets[_rr++ % _targets.size()]);
