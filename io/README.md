...
This directory contains examples demonstrating various features of the QB-IO library, which forms the asynchronous I/O
and utilities foundation for the QB C++ Actor Framework. These examples are designed to run as standalone applications
and illustrate how to use `qb-io` components directly.
...
* **Brief**: Demonstrates core asynchronous capabilities of the QB-IO library, including timer-driven operations, basic
  file I/O, and file system event watching. It is designed to run as a standalone application, showcasing `qb-io`
  features outside of the full `qb-core` actor system.
* **Details**:
    * **`FileProcessor`**:
    ...
    * **`FileWatcher`**:
        * Uses `ev::stat` (from `libev`, integrated with QB-IO's event loop) to monitor `qb_io_example.txt` for
          attribute changes (size, modification time).
    * **`main` function**:
...
* **To Run**:
  ...
  ```
  Observe the console output for file operations, timer ticks, and file change notifications. The program will run for a
  few seconds and then clean up.
...
* **Brief**: Demonstrates various file I/O operations, using `qb::io::system::file` for basic tasks and standard
  C++/POSIX calls for memory-mapped files and file statistics.
* **Details**:
    * The `FileOperationsManager` class orchestrates:
        1. **Writing Binary File**: Generates random binary data and writes it using `qb::io::system::file`. Measures
           throughput.
        2. **Reading Binary File**: Reads the file back using `qb::io::system::file`. Measures throughput.
        3. **Memory-Mapped I/O**: Uses POSIX `mmap()` to map a file to memory, writes data directly to the mapped
           region, and syncs with `msync()`.
...
* **To Run**:
  ...
  ```
  The program will create a test directory (`qb_fileio_test`), perform operations, print results and throughput, and
  then clean up.
...
* **Brief**: Demonstrates TCP networking with QB-IO, featuring an asynchronous server handling multiple clients and a
  client interacting via a text-based command protocol.
* **Details**:
    * **`TCPServer`**:
...
* **Brief**: Shows basic UDP client-server interaction using QB-IO's asynchronous framework, with a server echoing
  messages.
* **Details**:
    * **`UDPServer`**:
    ...
    * Server runs in the main thread, client in a new thread. Both use `qb::io::async::init()` and
...
* **Brief**: A comprehensive example of defining, implementing, and using a custom binary network protocol with QB-IO,
  including message framing, serialization, and parsing.
* **Details**:
    ...
    * **Serialization**:
        * `qb::allocator::pipe<char>::put<qb::custom_message>` specialization serializes `qb::custom_message` into the
          binary format.
    ...
    * **Server (`EchoServer`, `EchoServerClient`)**: Uses the custom protocol. Handles `HELLO` (sends `ACK`) and
      `ECHO_REQUEST` (sends `ECHO_REPLY`).
    * **Client (`EchoClient`)**: Uses the custom protocol. Sends `HELLO`, then `ECHO_REQUEST`s based on interactive user
      input. Manages response callbacks with timeouts.
...
* **To Run**:
    ...
    2. In another terminal, start the client:
       ...
       ```
       The client will connect, send a HELLO message, and then enter an interactive mode where you can type messages to
       be echoed by the server. Type "quit" to exit the client.

---

This README provides a guide to understanding and running the QB-IO examples. Each example is self-contained and focuses
on specific aspects of the library. 