...
void
printSectionHeader(const std::string &title) {
    qb::io::cout() << "\n=== " << title << " ===\n";
}
...
signalHandler(int signal) {
    ...
    qb::io::cout() << "Caught signal " << signal << " (" << (signal == SIGINT ? "SIGINT" : "SIGTERM") << ")" << std::endl;
    g_running = false;
}
} // namespace
...
public:
    ...
    on(qb::io::async::event::timer const &) {
        ...
        if (_operation_count >= _max_operations || !g_running) {
            qb::io::cout() << "FileProcessor: " << (_operation_count >= _max_operations ? "Completed all operations" : "Termination requested")
                           << ", stopping" << std::endl;
            return;
        }
    ...
    writeToFile() {
        ...
        _content = "Operation #" + std::to_string(_operation_count) + " at " + getCurrentTimestamp() + "\n"
        ...
        if (file.open(_filename, O_WRONLY | O_CREAT | O_TRUNC, 0644) >= 0) {
            ...
            if (bytes_written == static_cast<int>(_content.size())) {
                qb::io::cout() << "FileProcessor: Successfully wrote " << bytes_written << " bytes to file" << std::endl;
            } else {
                qb::io::cerr() << "FileProcessor: Partial write - only " << bytes_written << " of " << _content.size() << " bytes written"
                               << std::endl;
            }
        } else {
            qb::io::cerr() << "FileProcessor: Error opening file for writing: " << strerror(errno) << std::endl;
        }
    }
    ...
    readFromFile() {
        ...
        if (file.open(_filename, O_RDONLY) >= 0) {
            ...
            if (bytes_read > 0) {
                ...
                qb::io::cerr() << "FileProcessor: Error reading file: " << strerror(errno) << std::endl;
            }
        } else {
            qb::io::cerr() << "FileProcessor: Error opening file for reading: " << strerror(errno) << std::endl;
        }
    }
};
...
public:
    ...
    on(qb::io::async::event::timer const &) {
        _count++;
        qb::io::cout() << "TimerDemonstration: Tick #" << _count << " at " << getCurrentTimestamp() << std::endl;
...
public:
     ...
     */
    void
    onFileChange(ev::stat &watcher, int revents) {
        qb::io::cout() << "\nFileWatcher: Detected change in file: " << _filename << " at " << getCurrentTimestamp() << std::endl;

        // Check what changed
        if (revents & ev::STAT) {
            // File size changed
            if (watcher.attr.st_size != watcher.prev.st_size) {
                qb::io::cout() << "FileWatcher: File size changed from " << watcher.prev.st_size << " to " << watcher.attr.st_size << " bytes"
                               << std::endl;
            }
