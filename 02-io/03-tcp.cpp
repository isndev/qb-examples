...
void
printInfo(const std::string &component, const std::string &message) {
    qb::io::cout() << component << ": " << message << std::endl;
}

// Utility function to print error with component name
void
printError(const std::string &component, const std::string &message) {
...
private:
    ...
    void
    handleEchoCommand(const std::string &text) {
...
public:
    ...
    ~TCPServer() {
        printInfo("TCPServer", "Server shutting down, handled " + std::to_string(_connection_count) + " connections");
    }
    ...
    on(IOSession &session) {
        printInfo("TCPServer", "New connection from client " + std::to_string(++_connection_count));
    }
};
...
runClient() {
    ...
    printInfo("Client", "Connecting to server at " + std::string(SERVER_HOST) + ":" + std::to_string(SERVER_PORT));
    ...
    // Send test commands
    std::vector<std::string> commands = {"help", "time", "echo Hello from QB-IO client!", "stats", "unknown_command", "quit"};
