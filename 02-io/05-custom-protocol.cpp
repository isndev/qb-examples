...
template <>
pipe<char> &
pipe<char>::put<qb::custom_message>(const qb::custom_message &msg) {
...
public:
    ...
    getMessageSize() noexcept override {
        ...
        if (_reading_header) {
            ...
            qb::io::cout() << "DEBUG: Received header - Magic: 0x" << std::hex << _header.magic << " Version: 0x"
                           << static_cast<int>(_header.version) << " Type: 0x" << static_cast<int>(_header.type) << " ID: " << std::dec
                           << _header.id << " Length: " << _header.length << std::endl;

            // Validate magic and version
            if (_header.magic != qb_protocol::MAGIC || _header.version != qb_protocol::VERSION) {
                qb::io::cerr() << "Invalid header magic or version - Received Magic: 0x" << std::hex << _header.magic << " Version: 0x"
                               << static_cast<int>(_header.version) << std::dec << std::endl;
                reset();
                return 0;
            }
    ...
    void
    onMessage(std::size_t size) noexcept override {
        auto &buffer = this->_io.in();

        // Final safety check
        if (buffer.size() < HEADER_SIZE + _header.length) {
            qb::io::cerr() << "Buffer too small in onMessage: " << buffer.size() << " vs expected " << (HEADER_SIZE + _header.length)
                           << std::endl;
            reset();
            return;
        }
        ...
        qb::io::cout() << "DEBUG: Creating message object - Type: " << static_cast<int>(msg.type) << " ID: " << msg.id << " Payload: '"
...
public:
    ...
    on(Protocol::message &&msg) {
        qb::io::cout() << "Server received message, type: " << static_cast<int>(msg.type) << ", ID: " << msg.id << ", payload: " << msg.payload
                       << std::endl;
...
public:
    ...
    // Destructor
    ~EchoServer() {
        qb::io::cout() << "Server shutting down, handled " << _connections << " connection(s)" << std::endl;
    }
};
...
public:
    ...
    void
    on(Protocol::message &&msg) {
        qb::io::cout() << "Client received message, type: " << static_cast<int>(msg.type) << ", ID: " << msg.id << ", payload: " << msg.payload
                       << std::endl;
    ...
    void
    sendHello(std::function<void(bool)> on_connected = nullptr) {
    ...
    void
    sendEchoRequest(const std::string &data, std::function<void(bool success, const std::string &response)> callback, int timeout_ms = 5000) {
        if (!_connected) {
        ...
        qb::io::cout() << "DEBUG: Sending ECHO_REQUEST message, ID: " << msg_id << ", Payload: '" << data << "'" << std::endl;
        ...
        // Register callback for response with timeout
        {
            ...
            _callbacks[msg_id] = {
                ...
                std::chrono::steady_clock::now() + std::chrono::milliseconds(timeout_ms), true
            };
        }
