...
private:
    void
    scheduleTimer(const std::string &timer_name) {
        auto it = _timers.find(timer_name);
        if (it != _timers.end()) {
            // Schedule this timer to fire after its interval
            push<DelayedActionMsg>(id(), DelayedActionMsg::Action::TIMER_FIRE, it->second.interval.count(), timer_name);
        }
    }

    void
    fireTimer(const std::string &timer_name) {
        auto it = _timers.find(timer_name);
        if (it != _timers.end()) {
            ...
            qb::io::cout() << "Timer '" << info.name << "' fired, count: " << info.current_count << std::endl;

            // Check if we should stop the timer
            if (info.repeat_count > 0 && info.current_count >= info.repeat_count) {
                qb::io::cout() << "Timer '" << info.name << "' completed all " << info.repeat_count << " repetitions\n";
                _timers.erase(it);
