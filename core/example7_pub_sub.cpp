...
public:
    enum class Topic { WEATHER, NEWS, SPORTS, STOCK_PRICES, SYSTEM_STATUS };
...
public:
    ...
    explicit StepMessage(int step_num)
        : step(step_num) {}
};

// Helper function to convert Topic enum to string
std::string
topicToString(SubscribeMessage::Topic topic) {
    switch (topic) {
        case SubscribeMessage::Topic::WEATHER:
            return "WEATHER";
        case SubscribeMessage::Topic::STOCK_PRICES:
            return "STOCK_PRICES";
        case SubscribeMessage::Topic::NEWS:
            return "NEWS";
        case SubscribeMessage::Topic::SPORTS:
            return "SPORTS";
        case SubscribeMessage::Topic::SYSTEM_STATUS:
...
std::string
formatTopicSet(const std::set<SubscribeMessage::Topic> &topics) {
    std::string result = "[";
    bool        first  = true;
...
private:
    ...
    std::string
    topicToString(Topic topic) const {
        switch (topic) {
            case Topic::WEATHER:
                return "WEATHER";
            case Topic::NEWS:
...
public:
    ...
    on(SubscribeMessage &msg) {
        ...
        qb::io::cout() << "[Broker] New subscription: "
                       << "Actor " << msg.source_id << " subscribed to topic " << topicToString(msg.topic) << std::endl;
    }

    void
    on(UnsubscribeMessage &msg) {
        ...
        if (it != _topic_subscribers.end()) {
            ...
            qb::io::cout() << "[Broker] Subscription removed: "
                           << "Actor " << msg.source_id << " unsubscribed from topic " << topicToString(msg.topic) << std::endl;
        }
    }

    void
    on(PublishMessage &msg) {
        ...
        if (it != _topic_subscribers.end()) {
            ...
            for (const auto &subscriber_id : it->second) {
                ...
                push<MessageReceivedMessage>(subscriber_id, msg.topic, msg.content, msg.publisher, msg.timestamp);
    ...
    on(PrintStatisticsMessage &msg) {
        ...
        for (const auto &[subscriber_id, count] : _messages_per_subscriber) {
            qb::io::cout() << "  Actor " << subscriber_id << ": " << count << " messages received" << std::endl;
        }
        ...
        for (const auto &[topic, subscribers] : _topic_subscribers) {
            qb::io::cout() << "  " << topicToString(topic) << ": " << subscribers.size() << " subscribers" << std::endl;
        }
...
private:
    ...
    setupDefaultContent() {
        ...
        _topic_content[Topic::WEATHER].messages = {
            "Sunny, 25°C", "Cloudy with chance of rain, 18°C", "Heavy thunderstorms expected, 22°C", "Clear skies, 20°C",
            "Heatwave warning, 38°C"
        };

        // News
        _topic_content[Topic::NEWS].messages = {
            "New breakthrough in quantum computing announced", "Global climate summit ends with new agreements",
            "Stock markets hit record high", "Space mission successfully lands on Mars", "Major sports team wins championship"
        };

        // Sports
        _topic_content[Topic::SPORTS].messages = {
            "Team A wins against Team B with score 3-1", "Marathon record broken by athlete from Kenya",
            "Tennis tournament final scheduled for Sunday", "Swimmer breaks Olympic record", "Basketball player signs record contract"
        };

        // Stock prices
        _topic_content[Topic::STOCK_PRICES].messages = {
            "AAPL: $150.25 (+1.2%)", "MSFT: $305.10 (-0.5%)", "GOOG: $2,750.15 (+2.1%)", "AMZN: $3,400.50 (-1.7%)", "TSLA: $800.75 (+5.3%)"
        };

        // System status
        _topic_content[Topic::SYSTEM_STATUS].messages = {
            "All systems operational", "Database maintenance in progress", "High CPU usage detected", "Network latency increased",
    ...
    void
    publishMessageToTopic(Topic topic) {
        ...
        push<PublishMessage>(_broker_id, topic, content, _name, timestamp);
    }
    ...
    on(StepMessage &msg) {
        ...
        switch (msg.step) {
            case 1: {
                ...
                push<PublishMessage>(_broker_id, Topic::WEATHER, content, _name, timestamp);
                break;
            }
            ...
            case 2: {
                ...
                push<PublishMessage>(_broker_id, Topic::NEWS, content, _name, timestamp);
                break;
            }
            ...
            case 3: {
                ...
                push<PublishMessage>(_broker_id, Topic::SPORTS, content, _name, timestamp);
                break;
            }
            ...
            case 4: {
                ...
                push<PublishMessage>(_broker_id, Topic::STOCK_PRICES, content, _name, timestamp);
                break;
            }
            ...
            case 5: {
                ...
                push<PublishMessage>(_broker_id, Topic::SYSTEM_STATUS, content, _name, timestamp);
                break;
            }
...
private:
    ...
    struct MessageRecord {
        ...
        MessageRecord(Topic t, std::string c, std::string p, uint64_t pt)
            ...
            , published_timestamp(pt)
            , received_timestamp(getCurrentTimestamp()) {}
    };
...
public:
    SubscriberActor(std::string name)
        : _name(std::move(name)) {
    ...
    on(PrintHistoryMessage &msg) {
        ...
        for (int i = start_idx; i >= end_idx; i--) {
            ...
            qb::io::cout() << count + 1 << ". [" << topicToString(record.topic) << "] " << record.content << " (from " << record.publisher
...
public:
    DemoController(ActorId broker_id, ActorId publisher_id, const std::vector<ActorId> &subscriber_ids)
        : _broker_id(broker_id)
    ...
    publishMessages(int step) {
        ...
        switch (step) {
            case 0:
                // Publish a weather update
                push<PublishMessage>(_broker_id, Topic::WEATHER, "Direct weather update: Sunny, 25°C", "DemoController", getCurrentTimestamp());
                push<DelayedActionMessage>(id(), DelayedActionMessage::Action::PUBLISH_MESSAGES, 1);
                break;

            case 1:
                // Publish news
                push<PublishMessage>(_broker_id, Topic::NEWS, "Direct news: Breaking news about QB actors", "DemoController",
                                     getCurrentTimestamp());
            ...
            case 2:
                // Publish stock prices
                push<PublishMessage>(_broker_id, Topic::STOCK_PRICES, "Direct stock prices: QB stock up by 10%", "DemoController",
                                     getCurrentTimestamp());
...
main() {
    ...
    std::vector<ActorId> subscriber_ids = {news_reader_id, weather_watcher_id, stock_tracker_id};
