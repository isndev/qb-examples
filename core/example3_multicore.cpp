 ...
 * ...)`. (Note: Workers are on other cores but also register for this event. For true all-worker broadcast, iterating known worker IDs or using
 * `broadcast<T>()` would be typical).
 ...
 * - Broadcast Messaging: `push<EventType>(qb::BroadcastId(core_id), args...)` for core-specific broadcasts. (Actual system-wide broadcast is
 * `broadcast<T>(args...)`).
