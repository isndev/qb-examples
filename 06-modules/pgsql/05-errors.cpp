/**
 * @file examples/06-modules/pgsql/05-errors.cpp
 * @tier 06-modules
 * @teaches What failure looks like on each layer: a syntax error, a constraint violation, a NULL read
 *          as a value and a field read as the wrong type — each with the exception that names it.
 * @demonstrates qb::pg::tcp::database, qb::pg::params, qb::pg::error::db_error,
 *               qb::pg::error::value_is_null, qb::pg::error::field_type_mismatch, qb::pg::oid::int4,
 *               qb::pg::oid::text, qb::io::async::task<void>
 * @prerequisites 06-modules/pgsql/03-transactions
 * @expect "Successfully connected to PostgreSQL."
 * @expect "--- Running Error Handling Scenarios ---"
