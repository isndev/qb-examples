 ...
 * @tier 06-modules
 * @teaches A multi-statement transaction written straight-line inside one coroutine: begin, the
 *          statements, then commit or rollback on the branch that failed.
 * @demonstrates qb::pg::tcp::database, qb::pg::params, qb::pg::type_oid_sequence, qb::pg::oid::float8,
 ...
 * @expect "SUCCESS: Transaction for transferring "
...
main() {
    if (std::string(PG_CONNECTION_STRING) == "tcp://user:password@host:port[dbname]") {
        qb::io::cerr() << "WARNING: Using default PG_CONNECTION_STRING. "
                       << "Please update it in 03-transactions.cpp with your actual database details." << std::endl;
    }
