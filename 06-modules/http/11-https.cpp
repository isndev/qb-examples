/**
 * @file examples/06-modules/http/11-https.cpp
 * @tier 06-modules
 * @teaches TLS is a one-type switch: qb::http::ssl::Server<> instead of qb::http::Server<>, the same
 *          router and the same middleware over qb::http::ssl::DefaultSecureSession.
 * @demonstrates qb::http::ssl::Server<>, qb::http::ssl::DefaultSecureSession, qb::http::Server<>,
 *               router(), use, get, compile, listen, qb::http::CorsMiddleware<S>,
 *               qb::http::SecurityHeadersMiddleware<S>, qb::http::LoggingMiddleware<S>
 * @prerequisites 06-modules/http/08-static-files
 * @expect "Initializing HTTPS Server..."
 * @expect "=== QB HTTPS Server with SSL/TLS ==="
...
private:
    ...
    ensure_certificates_exist() {
        ...
        if (!std::filesystem::exists(_cert_file) || !std::filesystem::exists(_key_file)) {
            std::cerr << "SSL certificate not found (" << _cert_file
                      << "). Run the "
                         "example from its build output directory."
                      << std::endl;
            return false;
        }
    ...
    setup_https_error_handling() {
        ...
        error_handler->on_status(qb::http::Status::NOT_FOUND, [](auto ctx) {
            qb::json error_response = {
                {"error", "Not Found"}, {"message", "The requested HTTPS resource was not found"}, {"timestamp", std::time(nullptr)}
            };
    ...
    setup_secure_api_routes() {
        // Main page
        router().get("/", [this](auto ctx) { handle_main_page(ctx); });

        // Certificate info
        router().get("/certificate", [this](auto ctx) { handle_certificate_info(ctx); });

        // Security headers test
        router().get("/security/headers", [this](auto ctx) { handle_security_headers_test(ctx); });

        // Security test endpoint
        router().get("/security/test", [this](auto ctx) { handle_security_test(ctx); });
        ...
        api_group->use([](auto ctx, auto next) {
            ...
            if (api_key != "secure-api-key-12345") {
                ...
                qb::json error_response  = {{"error", "Invalid or missing API key"}, {"message", "Please provide a valid X-API-Key header"}};
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = error_response;
                ctx->complete();
                return;
        ...
        api_group->get("/user/profile", [this](auto ctx) { handle_secure_user_profile(ctx); });

        api_group->post("/payment/process", [this](auto ctx) { handle_secure_payment(ctx); });
        ...
        admin_group->use([](auto ctx, auto next) {
            ...
            if (admin_token != "admin-super-secret-token") {
                ...
                qb::json error_response  = {{"error", "Admin access denied"}, {"message", "Please provide a valid X-Admin-Token header"}};
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = error_response;
                ctx->complete();
                return;
        ...
        admin_group->get("/config", [this](auto ctx) { handle_admin_config(ctx); });

        api_group->post("/upload/secure", [this](auto ctx) { handle_secure_upload(ctx); });
    }

    void
    handle_main_page(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        qb::json response = {
            {"message", "Welcome to QB HTTPS Server"},
            {"version", "1.0"},
            {"ssl_enabled", true},
            {"security_features", {"HSTS enabled", "Secure headers", "SSL/TLS encryption", "API key authentication", "Admin token protection"}},
            {"endpoints",
             {{"GET /", "This main page"},
            ...
            {"authentication", {{"api_key", "X-API-Key: secure-api-key-12345"}, {"admin_token", "X-Admin-Token: admin-super-secret-token"}}},
            {"timestamp", std::time(nullptr)}
        };
    ...
    void
    handle_certificate_info(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        qb::json cert_info = {
    ...
    void
    handle_security_headers_test(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        ...
        qb::json security_analysis = {
            ...
            {"recommendations",
             {"All security headers are properly configured", "HSTS ensures HTTPS-only connections", "X-Frame-Options prevents clickjacking",
    ...
    void
    handle_security_test(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        ...
        qb::json security_test = {
            {"connection_security",
             {{"protocol", "HTTPS"}, {"encryption", "TLS/SSL"}, {"certificate_type", "self-signed"}, {"secure_connection", true}}},
            {"headers_security", {{"host_header", host}, {"user_agent_present", !user_agent.empty()}, {"secure_headers_applied", true}}},
            {"api_protection",
            ...
            {"recommendations",
             {"Connection is properly encrypted", "Security headers are configured", "API endpoints require authentication",
    ...
    void
    handle_secure_user_profile(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        ...
        qb::json profile = {
            ...
            {"security_settings", {{"two_factor_enabled", true}, {"api_key_active", true}, {"last_login", "2024-01-15T10:30:00Z"}}},
    ...
    void
    handle_secure_payment(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        try {
            ...
            qb::json payment_response = {
                ...
                {"security", {{"encrypted_connection", true}, {"pci_compliant", true}, {"fraud_check", "passed"}}},
            ...
            qb::json error_response  = {{"error", "Invalid payment data"}, {"message", e.what()}, {"timestamp", std::time(nullptr)}};
    ...
    void
    handle_admin_config(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        qb::json admin_config = {
            {"server_config",
             {{"https_port", 8443}, {"http_redirect_port", 8080}, {"ssl_certificate", "server.crt"}, {"ssl_private_key", "server.key"}}},
            {"security_config",
             {{"hsts_max_age", 31536000}, {"api_key_required", true}, {"admin_token_required", true}, {"secure_headers_enabled", true}}},
            {"performance_metrics",
    ...
    void
    handle_secure_upload(std::shared_ptr<qb::http::Context<qb::http::ssl::DefaultSecureSession>> ctx) {
        try {
            ...
            qb::json error_response  = {{"error", "Upload processing failed"}, {"message", e.what()}, {"timestamp", std::time(nullptr)}};
            ctx->response().status() = qb::http::Status::BAD_REQUEST;
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = error_response;
        }
    ...
    print_server_info() {
        ...
        std::cout << "  curl -k -H 'X-API-Key: secure-api-key-12345' -H 'X-Admin-Token: admin-super-secret-token' "
                     "https://localhost:8443/api/admin/config"
                  << std::endl;
