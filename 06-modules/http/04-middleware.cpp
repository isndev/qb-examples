/**
 * @file examples/06-modules/http/04-middleware.cpp
 * @tier 06-modules
 * @teaches Middleware as a chain: each hop either calls next() or answers, and a route group scopes
 *          the chain to one prefix instead of the whole server.
 * @demonstrates router(), use, group, get, compile, listen, qb::http::Context<S>,
 *               qb::http::DefaultSession, qb::http::Status::UNAUTHORIZED,
 *               qb::http::Status::TOO_MANY_REQUESTS, qb::http::Status::FORBIDDEN,
 *               qb::http::Method::OPTIONS
 * @prerequisites 06-modules/http/03-controllers
 * @expect "Middleware server listening on http://localhost:8080"
 * @expect "Initializing Middleware Demo Server Actor..."
...
private:
    ...
    setup_middleware() {
        ...
        router().use([](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx, std::function<void()> next) {
            ...
            std::cout << "[LOG] " << ctx->request().method() << " " << ctx->request().uri().path() << " from " << ctx->session()->ip()
                      << std::endl;
        ...
        router().use([](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx, std::function<void()> next) {
            ...
            if (start_time_opt.has_value()) {
                ...
                std::cout << "[TIMING] " << ctx->request().method() << " " << ctx->request().uri().path() << " completed in "
    ...
    setup_routes() {
        ...
        router().get("/", [this](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) { handle_home(ctx); });

        router().get("/public", [this](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) { handle_public(ctx); });
        ...
        // Protected routes
        protected_group->get("/profile", [this](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) { handle_profile(ctx); });

        protected_group->get("/data", [this](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) { handle_protected_data(ctx); });
        ...
        rate_limited_group->get("/", [this](std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) { handle_rate_limited(ctx); });
    }
    ...
    handle_home(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {
            ...
            {"endpoints",
             {"GET / - This home page", "GET /public - Public endpoint", "GET /api/profile - Protected endpoint (requires auth)",
              "GET /api/data - Protected data endpoint (requires auth)", "GET /limited/ - Rate limited endpoint (5 req/sec)"}},
            {"authentication", {{"header", "Authorization: Bearer <token>"}, {"valid_token", "secret-token-123"}}}
        };
    ...
    void
    handle_public(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ctx->response().status() = qb::http::Status::OK;
        ctx->response().add_header("Content-Type", "application/json");

        qb::json response = {
            {"message", "This is a public endpoint"}, {"accessible", "No authentication required"}, {"timestamp", std::time(nullptr)}
        };
    ...
    void
    handle_profile(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {
            {"user_id", user_id},
            {"user_name", user_name},
            {"profile", {{"email", "john.doe@example.com"}, {"role", "user"}, {"created_at", "2024-01-01T00:00:00Z"}}}
        };
    ...
    void
    handle_protected_data(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        auto        user_id_opt = ctx->get<std::string>("user_id");
        std::string user_id     = user_id_opt.value_or("unknown");
        ...
        qb::json response = {
            ...
            {"user_id", user_id},
            {"data", {{"sensitive_info", "This is protected data"}, {"balance", 1234.56}, {"permissions", {"read", "write"}}}}
        };
    ...
    void
    handle_rate_limited(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ctx->response().status() = qb::http::Status::OK;
        ctx->response().add_header("Content-Type", "application/json");
