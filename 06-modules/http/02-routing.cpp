/**
 * @file examples/06-modules/http/02-routing.cpp
 * @tier 06-modules
 * @teaches The router itself: literal paths, :params, wildcards and every verb, resolved by one
 *          radix tree that compile() builds once.
 * @demonstrates qb::http::Server<>, router(), get, post, put, del, compile, listen,
 *               qb::http::Context<S>, qb::http::DefaultSession, qb::http::Status::NOT_FOUND,
 *               qb::http::Status::CREATED, qb::http::Status::NO_CONTENT, qb::json
 * @prerequisites 06-modules/http/01-hello-server
 * @expect "Server listening on http://localhost:8080"
 * @expect "Available API endpoints:"
...
private:
    ...
    setup_routes() {
        // Root endpoint
        router().get("/", [this](auto ctx) { handle_root(ctx); });

        // API info endpoint
        router().get("/api", [this](auto ctx) { handle_api_info(ctx); });

        // Users collection endpoints
        router().get("/users", [this](auto ctx) { handle_get_users(ctx); });

        router().post("/users", [this](auto ctx) { handle_create_user(ctx); });

        // Individual user endpoints
        router().get("/users/:id", [this](auto ctx) { handle_get_user(ctx); });

        router().put("/users/:id", [this](auto ctx) { handle_update_user(ctx); });

        router().del("/users/:id", [this](auto ctx) { handle_delete_user(ctx); });

        // Parameterized greeting
        router().get("/hello/:name", [this](auto ctx) { handle_greeting(ctx); });

        // Query parameters example
        router().get("/search", [this](auto ctx) { handle_search(ctx); });

        // Wildcard route for file serving simulation
        router().get("/files/*path", [this](auto ctx) { handle_file_request(ctx); });
    }

    void
    handle_root(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {
            ...
            {"version", "1.0"},
            {"endpoints",
             {"GET /api - API information", "GET /users - List all users", "POST /users - Create a new user",
              "GET /users/:id - Get specific user", "PUT /users/:id - Update specific user", "DELETE /users/:id - Delete specific user",
              "GET /hello/:name - Personalized greeting", "GET /search?q=term - Search example", "GET /files/*path - File serving example"}}
        };
    ...
    handle_api_info(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {
            {"api", "QB HTTP Routing Demo API"}, {"version", "1.0"}, {"total_users", _users.size()}, {"next_user_id", _next_user_id}
        };
    ...
    void
    handle_get_users(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ctx->response().status() = qb::http::Status::OK;
        ctx->response().add_header("Content-Type", "application/json");
        ...
        qb::json response = {{"users", users_array}, {"count", _users.size()}};
    ...
    handle_create_user(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            ...
            // Create new user
            qb::json new_user = {
                {"id", _next_user_id}, {"name", user_data["name"]}, {"email", user_data["email"]}, {"age", user_data.value("age", 0)}
            };
    ...
    void
    handle_greeting(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {
            {"greeting", "Hello, " + name + "!"}, {"message", "Welcome to the QB HTTP framework!"}, {"timestamp", std::time(nullptr)}
        };
    ...
    handle_search(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        if (!search_term.empty()) {
            ...
            for (const auto &[id, user] : _users) {
                ...
                if (name.find(search_term) != std::string::npos || email.find(search_term) != std::string::npos) {
    ...
    void
    handle_file_request(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
