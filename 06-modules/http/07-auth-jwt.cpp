 ...
 * @expect "Initializing JWT Authentication Server..."
 * @expect "=== JWT Authentication Server ==="
...
struct UserAccount {
    ...
    qb::json
    to_json() const {
        return qb::json{{"username", username}, {"email", email}, {"roles", roles}, {"active", active}};
    }
};
...
private:
    ...
    setup_error_handling() {
        ...
        error_handler->on_status(qb::http::Status::FORBIDDEN, [](auto ctx) {
            qb::json error_response = {
                {"error", "Forbidden"}, {"message", "Insufficient permissions to access this resource."}, {"timestamp", std::time(nullptr)}
            };
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = error_response;
        });
    ...
    setup_routes() {
        ...
        router().post("/auth/login", [this](auto ctx) { handle_login(ctx); });

        router().post("/auth/register", [this](auto ctx) { handle_register(ctx); });
        ...
        // User info endpoint
        protected_group->get("/profile", [this](auto ctx) { handle_get_profile(ctx); });

        protected_group->put("/profile", [this](auto ctx) { handle_update_profile(ctx); });
        ...
        admin_group->use([](auto ctx, auto next) {
            ...
            if (!user_opt.has_value() || !user_opt.value().has_role("admin")) {
                ...
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Admin access required"}, {"message", "This endpoint requires admin privileges"}};
                ctx->complete();
                return;
            }
            next();
        ...
        admin_group->get("/users", [this](auto ctx) { handle_list_users(ctx); });

        admin_group->put("/users/:username/status", [this](auto ctx) { handle_update_user_status(ctx); });
        ...
        manager_group->use([](auto ctx, auto next) {
            ...
            auto user_opt = ctx->template get<qb::http::auth::User>("user");
            if (!user_opt.has_value() || (!user_opt.value().has_role("manager") && !user_opt.value().has_role("admin"))) {
                ctx->response().status() = qb::http::Status::FORBIDDEN;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() =
                    qb::json{{"error", "Manager access required"}, {"message", "This endpoint requires manager or admin privileges"}};
                ctx->complete();
                return;
            }
            next();
        ...
        manager_group->get("/reports", [this](auto ctx) { handle_get_reports(ctx); });

        // Logout endpoint (optional - client can just discard token)
        protected_group->post("/auth/logout", [this](auto ctx) { handle_logout(ctx); });

        // Token refresh endpoint
        protected_group->post("/auth/refresh", [this](auto ctx) { handle_refresh_token(ctx); });
    }
        ...
        qb::json info = {
            ...
            {"endpoints",
             {{"public",
              ...
              {"manager", {{"GET /api/manager/reports", "Access management reports"}}},
              {"admin", {{"GET /api/admin/users", "List all users"}, {"PUT /api/admin/users/:username/status", "Update user status"}}}}},
            {"authentication",
             {{"type", "JWT"}, {"header", "Authorization: Bearer <token>"}, {"algorithm", "HS256"}, {"expires_in", "1 hour"}}},
            {"test_users",
    ...
    void
    handle_login(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            ...
            if (!login_data.contains("username") || !login_data.contains("password")) {
                ...
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Missing credentials"}, {"message", "Username and password are required"}};
                ctx->complete();
                return;
            }
            ...
            if (!user_account.active) {
                ctx->response().status() = qb::http::Status::FORBIDDEN;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Account disabled"}, {"message", "Your account has been disabled"}};
                ctx->complete();
                return;
            }
            ...
            try {
                ...
                qb::json response = {
                    {"success", true}, {"message", "Login successful"}, {"token", token}, {"user", user_account.to_json()}, {"expires_in", 3600}
                    // 1 hour
                };
                ...
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Token generation failed"}, {"message", e.what()}};
            }
    ...
    void
    handle_register(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            auto register_data = ctx->request().body().as<qb::json>();

            if (!register_data.contains("username") || !register_data.contains("password") || !register_data.contains("email")) {
                ctx->response().status() = qb::http::Status::BAD_REQUEST;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Missing fields"}, {"message", "Username, password, and email are required"}};
                ctx->complete();
                return;
            }
            ...
            if (_users.find(username) != _users.end()) {
                ...
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "User exists"}, {"message", "Username already taken"}};
                ctx->complete();
                return;
            }
            ...
            qb::json response = {{"success", true}, {"message", "User registered successfully"}, {"user", new_user.to_json()}};
    ...
    void
    handle_get_profile(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        // Get authenticated user from context (set by auth middleware)
        auto user_opt = ctx->template get<qb::http::auth::User>("user");
        if (!user_opt.has_value()) {
            ctx->response().status() = qb::http::Status::UNAUTHORIZED;
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = qb::json{{"error", "No user information"}, {"message", "User not found in context"}};
            ctx->complete();
            return;
        }
        ...
        qb::json profile = {
            ...
            {"token_info", {{"valid", true}, {"remaining_time", "calculated_by_client"}}}
        };
    ...
    void
    handle_update_profile(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        auto user_opt = ctx->template get<qb::http::auth::User>("user");
        if (!user_opt.has_value()) {
            ctx->response().status() = qb::http::Status::UNAUTHORIZED;
            ctx->complete();
        ...
        try {
            ...
            qb::json response = {{"success", true}, {"message", "Profile updated successfully"}, {"user", user_it->second.to_json()}};
    ...
    handle_list_users(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {{"users", users_list}, {"total", _users.size()}};
    ...
    void
    handle_update_user_status(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            ...
            qb::json response = {{"success", true}, {"message", "User status updated"}, {"user", user_it->second.to_json()}};
    ...
    void
    handle_get_reports(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        // Manager-level endpoint
        qb::json reports = {
            {"user_stats",
             {{"total_users", _users.size()},
              {"active_users", std::count_if(_users.begin(), _users.end(), [](const auto &pair) { return pair.second.active; })},
            ...
            {"system_info",
             {{"uptime", "calculated_in_real_app"}, {"memory_usage", "calculated_in_real_app"}, {"active_sessions", "tracked_in_real_app"}}}
        };
    ...
    void
    handle_logout(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        qb::json response = {{"success", true}, {"message", "Logout successful"}, {"note", "Please discard your JWT token on the client side"}};
    ...
    void
    handle_refresh_token(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        auto user_opt = ctx->template get<qb::http::auth::User>("user");
        if (!user_opt.has_value()) {
            ctx->response().status() = qb::http::Status::UNAUTHORIZED;
            ctx->complete();
        ...
        try {
            ...
            qb::json response = {{"success", true}, {"message", "Token refreshed successfully"}, {"token", new_token}, {"expires_in", 3600}};
            ...
            ctx->response().status() = qb::http::Status::INTERNAL_SERVER_ERROR;
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = qb::json{{"error", "Token refresh failed"}, {"message", e.what()}};
        }
