/**
 * @file examples/06-modules/http/12-http2.cpp
 * @tier 06-modules
 * @teaches HTTP/2 over TLS+ALPN with the same router and the same middleware: the session and server
 *          types change, nothing above them does.
 * @demonstrates qb::http2::use<T>, qb::http::CorsMiddleware<S>, qb::http::LoggingMiddleware<S>,
 *               qb::http::static_files_middleware<S>, qb::http::StaticFilesOptions, router(), use,
 *               get, group, compile, listen, qb::io::uri
 * @prerequisites 06-modules/http/11-https
 * @expect "Initializing HTTP/2 Static File Server..."
 * @expect "HTTP/2 server actor created successfully"
...
class Http2StaticServer
    : public qb::Actor
    , public qb::http2::use<Http2StaticServer>::server<Http2StaticSession> {
private:
...
public:
    explicit Http2StaticServer(std::filesystem::path static_root = "./resources/http2")
        ...
        , _cert_file("resources/ssl/cert.pem")
        , _key_file("resources/ssl/key.pem") {}
...
private:
    ...
    setup_middleware() {
        ...
        router().use([](auto ctx, auto next) {
            ...
            ctx->response().add_header("Content-Security-Policy", "default-src 'self' 'unsafe-inline' 'unsafe-eval'; "
        ...
        auto logging_middleware = std::make_shared<qb::http::LoggingMiddleware<Http2StaticSession>>(
            [](qb::http::LogLevel level, const std::string &message) { std::cout << "[HTTP/2 INFO] " << message << std::endl; },
    ...
    setup_routes() {
        ...
        api_group->get("/multiplexing-demo", [](auto ctx) {
            ...
            response["benefits"]   = qb::json::array({"Reduced latency", "Better resource utilization", "Improved page load times"});
        ...
        api_group->get("/server-push-demo", [](auto ctx) {
            ...
            response["pushed_resources"] = qb::json::array({"/static/styles.css", "/static/http2-demo.js", "/static/data.json"});
