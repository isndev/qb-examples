/**
 * @file examples/06-modules/http/03-controllers.cpp
 * @tier 06-modules
 * @teaches A controller groups routes with the state they share, so a handler is a member function
 *          rather than a lambda closing over the server.
 * @demonstrates qb::http::Controller<S>, router(), controller<T>, get, post, put, del, use, compile,
 *               qb::http::Context<S>, qb::http::DefaultSession, qb::http::Server<>
 * @prerequisites 06-modules/http/02-routing
 * @expect "Controller server listening on http://localhost:8080"
 * @expect "Controller server actor created with ID: "
