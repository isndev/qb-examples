/**
 * @file examples/06-modules/http/05-rest-api-json.cpp
 * @tier 06-modules
 * @teaches A JSON REST service assembled from SHIPPED middleware — CORS, compression, security
 *          headers, logging, rate limit and an error chain — rather than hand-written equivalents.
 * @demonstrates qb::http::CorsMiddleware<S>, qb::http::CompressionMiddleware<S>,
 *               qb::http::SecurityHeadersMiddleware<S>, qb::http::LoggingMiddleware<S>,
 *               qb::http::RateLimitMiddleware<S>, qb::http::MiddlewareTask<S>,
 *               set_error_task_chain, router(), use, group, compile, qb::json
 * @prerequisites 06-modules/http/04-middleware
 * @expect "=== REST API Server with JSON ==="
...
struct Book {
    ...
    qb::json
    to_json() const {
        return qb::json{{"id", id},     {"title", title},         {"author", author}, {"isbn", isbn},
                        {"year", year}, {"available", available}, {"price", price},   {"categories", categories}};
    }
...
private:
    ...
    setup_standard_middleware() {
        ...
        auto compression_options =
            qb::http::CompressionOptions().compress_responses(true).decompress_requests(true).min_size_to_compress(1024).preferred_encodings(
                {"gzip", "deflate"});
    ...
    setup_error_handling() {
        ...
        error_handler->on_status(qb::http::Status::BAD_REQUEST, [](auto ctx) {
            qb::json error_response = {{"error", "Bad Request"}, {"message", "Invalid request data"}, {"timestamp", std::time(nullptr)}};
        ...
        error_handler->on_status(qb::http::Status::NOT_FOUND, [](auto ctx) {
            qb::json error_response = {
                {"error", "Not Found"}, {"message", "The requested resource was not found"}, {"timestamp", std::time(nullptr)}
            };
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = error_response;
        });
        ...
        error_handler->on_status(qb::http::Status::METHOD_NOT_ALLOWED, [](auto ctx) {
            qb::json error_response = {
                {"error", "Method Not Allowed"}, {"message", "HTTP method not supported for this endpoint"}, {"timestamp", std::time(nullptr)}
            };
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = error_response;
        });
        ...
        error_handler->on_status(qb::http::Status::TOO_MANY_REQUESTS, [](auto ctx) {
            qb::json error_response = {
                {"error", "Too Many Requests"}, {"message", "Rate limit exceeded. Please try again later."}, {"timestamp", std::time(nullptr)}
            };
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = error_response;
        });
        ...
        error_handler->on_status_range(500, 599, [](auto ctx) {
            qb::json error_response = {
                {"error", "Internal Server Error"}, {"message", "An unexpected error occurred"}, {"timestamp", std::time(nullptr)}
            };
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = error_response;
        });
    ...
    setup_routes() {
        ...
        router().get("/", [this](auto ctx) { handle_api_documentation(ctx); });

        // Health check endpoint
        router().get("/health", [this](auto ctx) { handle_health_check(ctx); });

        // Books API endpoints
        auto api_group = router().group("/api/v1");

        // GET /api/v1/books - List all books with filtering
        api_group->get("/books", [this](auto ctx) { handle_get_books(ctx); });

        // POST /api/v1/books - Create new book
        api_group->post("/books", [this](auto ctx) { handle_create_book(ctx); });

        // GET /api/v1/books/:id - Get specific book
        api_group->get("/books/:id", [this](auto ctx) { handle_get_book(ctx); });

        // PUT /api/v1/books/:id - Update book
        api_group->put("/books/:id", [this](auto ctx) { handle_update_book(ctx); });

        // DELETE /api/v1/books/:id - Delete book
        api_group->del("/books/:id", [this](auto ctx) { handle_delete_book(ctx); });

        // PATCH /api/v1/books/:id - Partial update
        api_group->patch("/books/:id", [this](auto ctx) { handle_patch_book(ctx); });

        // Search endpoint
        api_group->get("/books/search", [this](auto ctx) { handle_search_books(ctx); });

        // Statistics endpoint
        api_group->get("/stats", [this](auto ctx) { handle_get_stats(ctx); });
    }

    void
    initialize_sample_data() {
        // Add some sample books
        _books[_next_id++] = {1,     "The C++ Programming Language",     "Bjarne Stroustrup", "978-0321563842", 2013, true,
                              79.99, {"Programming", "C++", "Technical"}};
        _books[_next_id++] = {
            2, "Clean Code", "Robert C. Martin", "978-0132350884", 2008, true, 49.99, {"Programming", "Software Engineering"}
        };
        ...
        _books[_next_id++] = {
            5, "The Pragmatic Programmer", "Andy Hunt", "978-0135957059", 2019, true, 54.99, {"Programming", "Software Engineering"}
        };
    }

    void
    handle_api_documentation(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        qb::json api_docs = {
            {"name", "Books REST API"},
            {"version", "1.0"},
    ...
    void
    handle_health_check(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        qb::json health = {
            {"status", "healthy"},
    ...
    void
    handle_get_books(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        ...
        for (const auto &[id, book] : _books) {
            // Apply filters
            if (!author_filter.empty() && book.author.find(author_filter) == std::string::npos)
                continue;
            if (!available_filter.empty() && std::to_string(book.available) != available_filter)
                continue;
        ...
        qb::json response = {
            {"books", books},
            {"pagination", {{"offset", offset}, {"limit", limit}, {"total", static_cast<int>(_books.size())}, {"returned", added_count}}},
            {"filters", {{"author", author_filter}, {"available", available_filter}, {"category", category_filter}}}
        };
    ...
    void
    handle_create_book(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            ...
            if (!json_body.contains("title") || !json_body.contains("author")) {
                ...
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() =
                    qb::json{{"error", "Validation Error"}, {"message", "Missing required fields: title and author are required"}};
                ctx->complete();
                return;
            }
            ...
            if (new_book.title.empty() || new_book.author.empty()) {
                ctx->response().status() = qb::http::Status::BAD_REQUEST;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Validation Error"}, {"message", "Title and author cannot be empty"}};
                ctx->complete();
                return;
            }
            ...
            if (new_book.year < 0 || new_book.year > 2025) {
                ctx->response().status() = qb::http::Status::BAD_REQUEST;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Validation Error"}, {"message", "Year must be between 0 and 2025"}};
                ctx->complete();
                return;
            }
            ...
            if (new_book.price < 0) {
                ctx->response().status() = qb::http::Status::BAD_REQUEST;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Validation Error"}, {"message", "Price cannot be negative"}};
                ctx->complete();
                return;
            }
            ...
            qb::json response = {{"message", "Book created successfully"}, {"book", new_book.to_json()}};
    ...
    void
    handle_get_book(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            ...
            if (!parsed_id) {
                ctx->response().status() = qb::http::Status::BAD_REQUEST;
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() = qb::json{{"error", "Invalid ID"}, {"message", "Book ID must be a valid integer"}};
                ctx->complete();
                return;
            }
            ...
            if (it == _books.end()) {
                ...
                ctx->response().add_header("Content-Type", "application/json");
                ctx->response().body() =
                    qb::json{{"error", "Not Found"}, {"message", "Book with ID " + std::to_string(book_id) + " not found"}};
                ctx->complete();
                return;
            }
    ...
    void
    handle_update_book(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            // Untrusted path param: parse without throwing, reject a non-integer id with 400.
            auto parsed_id = qb::to_number<int>(ctx->path_param("id"));
            if (!parsed_id) {
            ...
            qb::json response = {{"message", "Book updated successfully"}, {"book", updated_book.to_json()}};
    ...
    void
    handle_patch_book(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            // Untrusted path param: parse without throwing, reject a non-integer id with 400.
            auto parsed_id = qb::to_number<int>(ctx->path_param("id"));
            if (!parsed_id) {
            ...
            if (json_body.contains("title"))
                book.title = json_body["title"];
            if (json_body.contains("author"))
                book.author = json_body["author"];
            if (json_body.contains("isbn"))
                book.isbn = json_body["isbn"];
            if (json_body.contains("year"))
                book.year = json_body["year"];
            if (json_body.contains("available"))
                book.available = json_body["available"];
            if (json_body.contains("price"))
                book.price = json_body["price"];
            if (json_body.contains("categories"))
    ...
    void
    handle_delete_book(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        try {
            // Untrusted path param: parse without throwing, reject a non-integer id with 400.
            auto parsed_id = qb::to_number<int>(ctx->path_param("id"));
            if (!parsed_id) {
            ...
            qb::json response = {{"message", "Book deleted successfully"}, {"deleted_book", deleted_book.to_json()}};
    ...
    void
    handle_search_books(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
        auto query = ctx->request().query("q");
        if (query.empty()) {
            ctx->response().status() = qb::http::Status::BAD_REQUEST;
            ctx->response().add_header("Content-Type", "application/json");
            ctx->response().body() = qb::json{{"error", "Missing Parameter"}, {"message", "Search query parameter 'q' is required"}};
            ctx->complete();
            return;
        }
        ...
        for (const auto &[id, book] : _books) {
            ...
            if (title_lower.find(search_term) != std::string::npos || author_lower.find(search_term) != std::string::npos
                || book.isbn.find(search_term) != std::string::npos) {
                results.push_back(book.to_json());
            }
        }

        qb::json response = {{"query", query}, {"results", results}, {"count", results.size()}};
    ...
    void
    handle_get_stats(std::shared_ptr<qb::http::Context<qb::http::DefaultSession>> ctx) {
