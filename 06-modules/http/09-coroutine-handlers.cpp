/**
 * @file examples/06-modules/http/09-coroutine-handlers.cpp
 * @tier 06-modules
 * @teaches A route handler that is a coroutine: await a sleep, await an outbound request, await two
 *          at once with when_all, and return the reply — no callback, no continuation, no state bag.
 * @demonstrates qb::io::async::task<void>, qb::io::async::sleep, qb::io::async::when_all,
 *               qb::http::GET, qb::http::Request, qb::http::async::Reply, router(), get, compile,
 *               qb::http::Server<>, qb::http::Context<S>
 * @prerequisites 03-coroutines/02-actor-coroutines, 06-modules/http/03-controllers
 * @expect "Coroutine server on http://localhost:8080"
 * @expect "Initializing Coroutine-Handlers HTTP Server..."
