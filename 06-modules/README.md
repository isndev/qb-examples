...
### 1. HTTP Module (`./http/`) — 14 programs
...
* **Content**: `01-hello-server`, `02-routing`, `03-controllers`, `04-middleware`, `05-rest-api-json`,
  ...
  `12-http2`, `13-http3` (one dual-stack server on TCP **and** QUIC) and `14-streaming-and-cookies`
  (chunked framing, SSE, cookies and the `CookieJar`, forms and multipart, HTTP dates and
  conditional `304`s).
* **Gated**: `07`, `11` and `12` declare `REQUIRES ssl`; `13` guards itself with `#ifdef QBM_HTTP_HAS_HTTP3`
  because "nghttp3 is present" is not a condition the `REQUIRES` vocabulary can express.
...
### 3. PostgreSQL Module (`./pgsql/`) — 8 programs
...
* **Content**: `01-connect-and-query`, `02-parameters` (prepared statements with explicit parameter OIDs),
  ...
  event bus, with a `notify_co_consumer` you `co_await`) and `08-tls-and-limits` (STARTTLS on the same port).
...
### 4. Redis Module (`./redis/`) — 10 programs
...
* **Content**: `01-connect`, `02-data-types` (string, hash, list, set — the merge target of the two retired
  ...
  and `10-cache-actor` (the client as an actor member).
