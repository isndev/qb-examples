/**
 * @file examples/06-modules/ws/01-chat-server.cpp
 * @tier 06-modules
 * @teaches A WebSocket server and an HTTP server side by side: the HTTP one serves the page, the
 *          WebSocket one owns the sockets that the page upgrades onto.
 * @demonstrates qb::http::ws::protocol<S>, qb::http::ws::MessageText, qb::http::use<T>,
 *               qb::io::use<T>, qb::http::Request, qb::http::Response,
 *               qb::http::StaticFilesMiddleware<S>, qb::http::CorsMiddleware<S>,
 *               qb::http::LoggingMiddleware<S>, router(), use, get, compile, listen
 * @prerequisites 06-modules/http/08-static-files
 * @expect "Starting WebSocket Chat Server..."
 * @expect "HTTP Server running on http://localhost:"
...
private:
    ...
    void
    init_message_handlers() {
        _message_handlers[MessageType::MESSAGE] = [this](const qb::json &msg) {
            handle_chat_message(msg);
        };

        _message_handlers[MessageType::USER_JOINED] = [this](const qb::json &msg) {
            handle_user_joined(msg);
        };

        _message_handlers[MessageType::USERNAME_UPDATE] = [this](const qb::json &msg) {
            handle_username_update(msg);
        };
    }
    ...
    // ========================================================================
    void
    handle_chat_message(const qb::json &message_json) {
    ...
    void
    handle_user_joined(const qb::json &message_json) {
        ...
        if (!_user_announced) {
            ...
            // Notify other users
            auto response        = create_message(MessageType::USER_JOINED, message_json.value("timestamp", 0ULL));
            response["username"] = username;
    ...
    void
    handle_username_update(const qb::json &message_json) {
        ...
        if (new_username != old_username && _user_announced) {
            ...
            auto response            = create_message(MessageType::USERNAME_CHANGED, message_json.value("timestamp", 0ULL));
    ...
    extract_chat_message_data(const qb::json &msg) {
        return {msg.value("username", "Anonymous"), msg.value("message", ""), msg.value("timestamp", 0ULL)};
    }
    ...
    // ========================================================================
    bool
    validate_chat_message(const std::string &message) {
        if (message.empty()) {
        ...
        if (message.length() > Validation::MAX_MESSAGE_LENGTH) {
            send_error("Message too long (max " + std::to_string(Validation::MAX_MESSAGE_LENGTH) + " characters)");
            return false;
        }
    ...
    validate_username(const std::string &username) {
        if (username.empty() || username.length() > Validation::MAX_USERNAME_LENGTH) {
            send_error("Invalid username (" + std::to_string(Validation::MIN_USERNAME_LENGTH) + "-"
    ...
    // ========================================================================
    qb::json
    create_message(const std::string &type, uint64_t timestamp = 0) {
        return qb::json{{"type", type}, {"timestamp", timestamp == 0 ? qb::unix_nanos(qb::wall_now()) : timestamp}};
    }
    ...
    // ========================================================================
    void
    send_welcome_message(const std::string &username) {
    ...
    void
    send_ws_message(const std::string &message) {
    ...
    void
    send_error(const std::string &error_message) {
    ...
    void
    broadcast_to_all(const std::string &message) {
        broadcast_message(message, nullptr);
    }

    void
    broadcast_to_others(const std::string &message) {
...
public:
    explicit HttpServer(std::filesystem::path static_root, uint16_t port, qb::ActorId chat_server_id)
        ...
        , _port(port)
        , _chat_server_id(chat_server_id) {}
...
private:
    ...
    std::shared_ptr<qb::http::LoggingMiddleware<HttpSession>>
    ...
    std::shared_ptr<qb::http::StaticFilesMiddleware<HttpSession>>
    ...
    void
    setup_main_routes() {
        router().get("/", [](auto ctx) { ctx->redirect("/static/index.html"); });
    }
    ...
    void
    setup_websocket_route() {
        router().get("/ws", [this](auto ctx) { handle_websocket_upgrade(ctx); });
    }
    ...
    create_api_info() {
        return qb::json{
            ...
            {"features",
             {"websocket_chat", "static_file_serving", "real_time_messaging", "actor_based_architecture", "separated_responsibilities",
              "clean_message_dispatch"}},
            {"endpoints", {{"chat_interface", "/"}, {"websocket", "/ws"}, {"api_info", "/api/info"}, {"health_check", "/health"}}},
            {"static_files",
    ...
    qb::json
    create_health_info() {
        return qb::json{{"status", "healthy"}, {"server_type", "HTTP"}, {"timestamp", qb::unix_nanos(qb::wall_now())}};
    }
    ...
    void
    transfer_session_to_websocket(Transport &&transport, ContextPtr ctx) {
    ...
    template <typename ContextPtr>
    void
    send_websocket_upgrade_error(ContextPtr ctx) {
...
// ============================================================================
void
ChatSession::broadcast_message(const std::string &message, ChatSession *exclude) {
    qb::http::ws::MessageText ws_message;
    ws_message << message;
...
// Utility Functions
// ============================================================================
std::pair<std::filesystem::path, uint16_t>
