/**
 * @file examples/01-actors/05-lifecycle.cpp
 * @tier 01-actors
 * @teaches How an actor ends: a KillEvent handler that runs only because it was registered, a
 *          two-phase drain that finishes the work in flight first, and the difference between
 *          "stop taking work" and "terminate".
 * @demonstrates registerEvent<E>, qb::KillEvent, broadcast<qb::KillEvent>, kill(), push<E>,
 *               getSource(), spawn, qb::ScopedCoroContext, ctx.sleep, qb::Main, addActor<T>
 * @prerequisites 01-actors/02-messaging
 * @expect "] Supervisor: Reached max status checks, initiating shutdown"
 * @expect "] Supervisor: All workers drained, broadcasting KillEvent"
 * @expect "] Main: All actors have terminated, exiting"
 *
