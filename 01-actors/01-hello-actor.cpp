/**
 * @file examples/01-actors/01-hello-actor.cpp
 * @tier 01-actors
 * @teaches The smallest complete qb program: an actor that subscribes to one event and one that
 *          sends it on every turn of the core's loop, plus the engine that runs both and the two
 *          ways an actor ends.
 * @demonstrates qb::Actor, qb::Event, qb::ActorId, qb::io::async::task<bool>, registerEvent<E>,
 *               push<E>, kill(), qb::ICallback, registerCallback, qb::LoopEvent, qb::Main,
 *               addActor<T>, qb::io::cout
 * @prerequisites none
 * @expect "SimpleActor: Initialized"
 * @expect "SenderActor: Terminating"
